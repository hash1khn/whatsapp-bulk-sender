...
const PLACEHOLDERS = ['MAKE', 'MODEL', 'YEAR', 'VIN', 'PARTS', 'DETAILS', 'GALLERY', 'QTY', 'PART NAME', 'PART NUMBER'];

function parseContentEditable(html) {
  // Replace chips with [PLACEHOLDER] text
  const div = document.createElement('div');
  div.innerHTML = html;
  div.querySelectorAll('span[data-ph]').forEach(span => {
    span.outerHTML = `[${span.getAttribute('data-ph')}]`;
  });
  let text = div.innerHTML.replace(/<div>|<br>/gi, '\n');
  div.innerHTML = text;
...
function renderPreviewFromHtml(raw, mockData) {
  // Apply smart placeholder logic but preserve all user spacing and formatting
  ...
  // Convert newlines to <br> (do NOT replace spaces)
  text = text.replace(/\n/g, '<br>');
  // Highlight chips (for any remaining [PLACEHOLDER])
...
export default function MessageEditorConcept() {
  ...
  // Store the last known value for preview and sending
  const [lastValue, setLastValue] = useState(initialText);

  // Insert a placeholder chip at the cursor
  const insertPlaceholder = (ph) => {
    ...
    sel.removeAllRanges();
    sel.addRange(range);
    // Update lastValue for preview
    setTimeout(() => {
      setLastValue(editorRef.current.innerText);
    }, 0);
  };

  // On blur, update lastValue
  const handleBlur = () => {
  ...
  // On test send, get current value
  ...
  // Handle mock data input
  const handleMockChange = (ph, value) => {
    setMockData(prev => ({ ...prev, [ph]: value }));
  };

  // For preview and mock data form, always use the latest value
    ...
    <div className="flex flex-col items-center min-h-screen py-10 bg-gray-50">
      ...
      <Card className="w-full max-w-2xl">
        ...
        <CardContent>
          ...
          />
          <Button onClick={handleTestSend} disabled={isSending} className="mt-4 bg-blue-600 hover:bg-blue-700 text-white">
